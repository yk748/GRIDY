...
#-----------------------------------------------------------------------------#
# Simulation 1: Size of problem
#-----------------------------------------------------------------------------#
# Problem size
d_list <- c(200,400,100,100,100,100)
T_list <- c(100,100,200,400,200,200)
K_list <- c(50,50,50,50,10,100)
  ...
  for (set in 1:2){
    ...
    for (iter in 1:max_iter){
      ...
      GRIDY_refitted <- factor_regression(KK,r_J,r_G,model_dgp,
                                          ...
                                          SCA_PF2_Group1,
                                          SCA_PF2_Group2)
      GRIDY_YK <- YK_compute(dd,TT,KK,r_J,r_G,Control_param,model_dgp,GRIDY_refitted)
      ...
      SCA_P_refitted <- factor_regression(KK,r_J,r_G,model_dgp,
                                          ...
                                          SCA_P_Group1,
                                          SCA_P_Group2)
      SCA_P_YK <- YK_compute(dd,TT,KK,r_J,r_G,Control_param,model_dgp,SCA_P_refitted)
      ...
      GICA_refitted <- factor_regression(KK,r_J,r_G,model_dgp,
                                         ...
                                         GICA_Group1,
                                         GICA_Group2)
      GICA_YK <- YK_compute(dd,TT,KK,r_J,r_G,Control_param,model_dgp,GICA_refitted)
      ...
      Unfitted_SCA_PF2_YK <- YK_compute(dd,TT,KK,r_J,r_G,Control_param,model_dgp,Factor_unfitted_SCA_PF2)
      ...
      Unfitted_GICA_YK <- YK_compute(dd,TT,KK,r_J,r_G,Control_param,model_dgp,Factor_unfitted_GICA)
      ...
      DSCA_refitted <- factor_regression(KK,r_J,r_G,model_dgp,
                                         ...
                                         DSCA_Group1,
                                         DSCA_Group2)
      DSCA_YK <- YK_compute(dd,TT,KK,r_J,r_G,Control_param,model_dgp,DSCA_refitted)
      
      DGICA_refitted <- factor_regression(KK,r_J,r_G,model_dgp,
                                          DGICA_Joint,
                                          DGICA_Group1,
                                          DGICA_Group2)
      DGICA_YK <- YK_compute(dd,TT,KK,r_J,r_G,Control_param,model_dgp,DGICA_refitted)
      ...
      # ----------------------------------------------------------- #
      cat("Currently ",iter,"th iteration is done.\n")
    }
    
    file_name <- paste0("sim1_type",set,"_d",dd,"T",TT,"K",KK,".RData")
...
#-----------------------------------------------------------------------------#
# Simulation 2: Strengths of signals
#-----------------------------------------------------------------------------#
# Problem size
dd <- 100
TT <- 200
...
for (case in 1:6){
  ...
  for (set in 1:2){
    ...
    file_name <- paste0("sim2_type",set,"_snr",case,".RData")
    save( list_GRIDY,list_SCA_P,list_GICA,list_DSCA,list_DGICA,list_Unfitted_SCA_PF2,list_Unfitted_GICA,
          file=file_name)
  }
}
...
# -----------------------------------------------------------------------------#
# Simulation 3: Initial rank selection
# -----------------------------------------------------------------------------#
sigma_eps <- 1
...
for (case in 1:5){
  ...
  for (set in 1:6){
    ...
    for (r in 1:10){
      ...
      Control_param <- data.frame(type=type,sigma_xi=sigma_xi,J_per=50,
                                  b_bar_min=0,b_bar_max=1,
                                  b_tilde_min=0,b_tilde_max=1,
                                  sigma_c=sigma_c,sigma_eps=sigma_eps)
      
      model_dgp <- sim_model(r_J, r_G, dd, (TT+H), KK, Control_param)
    ...
    file_name <- paste0("Rank_sel_snr",set,"_d",dd,"T",TT,"K",KK,".RData")
